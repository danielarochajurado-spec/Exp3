    ...
    document.addEventListener('DOMContentLoaded', () => {    
      ...
      const instructions = {    
        type: jsPsychHtmlButtonResponse,   
          stimulus: `<h2>Bem-vindo!</h2>
            <p>
              Nesta tarefa, você jogará um jogo em que você deverá eliminar gemas. 
              Para isso, clique em duas gemas adjacentes para trocar suas posições.
              A troca funciona ao juntar pelo menos três gemas consecutivas da mesma cor.
              <br><br> 
              O jogo será interrompido continuamente. Você terá que escolher entre esperar 
              pouco para jogar pouco ou esperar mais para jogar três vezes mais.
      ...
      <div class="board-wrap">         
        ...
        <aside class="hud">       
          ...
          <p class="muted">Tip: clique em dois gemas vizinhas para troca-las de posição. A troca funciona quando tem pelo menos três gemas continuas da mesma cor.</p>
          </aside>   
          </div> 
          `, 
          choices: [],  
  ...
  return [participant_id_trial, instructions, initialMatch3Trial];
      ...
      function freeTrial(delayState, current_delay_small, condition_index, response_type, trial_number_tracker, p_tracker) {  
        return {
          ...
          choices: () => [
            `Esperar ${(delayState.large / 1000).toFixed(1)}s para jogar por um tempo maior`,
            `Esperar ${(current_delay_small / 1000).toFixed(1)}s para jogar por um tempo menor`
          ],
          button_html: [
      ...
      function createMatch3Trial() {
        return {
            ...
            on_load: () => {
                ...
                const currentScore = window.gameState?.score ?? 0;
                const newLevel = Math.floor(currentScore / 200) + 1; // Example: 200 points per level
                const newAchievements = Math.floor(currentScore / 200)+1; // Example: 20 points per achievement

                // Update the grid size based on the level
                const newGridSize = Math.min(14, 5 + Math.floor(newLevel / 2)); // Example: Start at 5x5, max at 10x10
          ...
          stimulus: () => {
            ...
            // HTML del trial con score arriba de las opciones
              return `
                ...
                <div class="board-wrap">
                  <div class="center-content">
                    <p style="text-align: center; color: white;">Escolha a única opção</p>
                    </div>
                  </div>
                <aside class="hud">
                  <h1>Caça-cores</h1>
      ...
      const final_message = {
        ...
        stimulus: () => {
          ...
          return `<p>Tarefa concluída! Você fez ${finalScore} pontos.</p><p>Obrigada por participar. Por favor, continue para a próxima tarefa.</p>`;
        },
        choices: ['Fechar']
      };
